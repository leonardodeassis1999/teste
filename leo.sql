...
select c.nome, e.rua, e.numero, e.bairro from cliente c inner join endereco e on c.id_cliente = e.id_cliente;
select count(*) total_pedidos from pedidos;

update pedidos
set status_pedidos = 'Entregue'
where id_pedido = 3;
alter table pedidos
add column descontos decimal(5,20) default 0.00;
select * from pedidos;